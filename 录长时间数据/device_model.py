...
class DeviceModel:
    ...
    def __init__(self, deviceName, BLEDevice, callback_method=None):
        ...
        self.isOpen = False
        # 如果没有提供回调方法，使用默认的打印方法
        if callback_method is None:
            self.callback_method = self._default_callback
        else:
            self.callback_method = callback_method
        self.deviceData = {}
        # 加速度数据队列 Acceleration data queue
        self.acc_data_queue = queue.Queue()
        # 所有数据队列 All data queue
        self.all_data_queue = queue.Queue()
    ...
    # 默认回调方法 Default callback method
    def _default_callback(self, device):
        # 打印设备数据 Print device data
    ...
    def processData(self, Bytes):
        ...
        if Bytes[1] == 0x61:
            ...
            # 将加速度数据放入队列 Put acceleration data into queue
            self.acc_data_queue.put({
                'AccX': round(Ax, 3),
                'AccY': round(Ay, 3),
                'AccZ': round(Az, 3)
            })
            # 将完整数据放入all_data_queue Put complete data into all_data_queue
            data_frame = {
                'timestamp': timestamp,
                'AccX': round(Ax, 3),
                'AccY': round(Ay, 3),
            ...
            self.all_data_queue.put(data_frame)
